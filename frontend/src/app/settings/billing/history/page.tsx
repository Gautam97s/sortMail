...
export default function BillingHistoryPage() {
    ...
    const { data: transactions = [], isLoading } = useQuery<Transaction[]>({
        queryKey: ["credits-transactions", page, "history"],
        queryFn: async () => {
            ...
            <Card>
                <CardContent className="p-0">
                                    ...
                                    transactions.map((t) => (
                                        <tr key={t.id} className="hover:bg-paper-mid/30 transition-colors">
                                            ...
                                            <td className="px-6 py-3 text-right">
                                                <span className="text-sm font-mono text-ink-light">{t.balance_after}</span>
                                            </td>
                                        </tr>
                                    ))
                                )}
                    ...
                    <div className="p-4 border-t border-border-light flex items-center justify-between">
                        <span className="text-xs text-muted">Showing page {page + 1}</span>
