...
function DashboardContent() {
    ...
    return (
        <div className="flex flex-col p-4 md:p-6 gap-6 max-w-[1280px] mx-auto">
            {/* Header Greeting */}
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* AI Briefing Card */}
                <div className="lg:col-span-2 bg-gradient-to-br from-tertiary-fixed/40 to-primary-fixed/20 rounded-2xl p-5 md:p-6 tonal-shadow relative overflow-hidden border border-white/40">
                    ...
                    <div className="relative z-10 flex flex-col gap-4 md:pr-12">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                            {briefing.suggested_actions.map((action: string, i: number) => (
                                <button key={i} className="px-3 py-1.5 bg-white/80 hover:bg-white text-on-surface border border-outline-variant/30 transition-all rounded-full text-[11px] font-bold shadow-sm hover:scale-105 active:scale-95">
                                    {action}
                                </button>
                            ))}
                        </div>
                ...
                <div className="bg-surface-container-low rounded-2xl p-5 border border-outline-variant/10 flex flex-col justify-between tonal-shadow">
                    <div className="flex flex-col gap-1">
                        <span className="text-[11px] font-bold text-outline uppercase tracking-[0.2em] mb-2">Focus Score</span>
                        <div className="flex items-end gap-2">
                            <span className="text-4xl font-headline font-bold text-primary tracking-tighter">88</span>
                    ...
                    <Link href="/search" className="mt-5 flex items-center justify-between w-full p-3 bg-white rounded-xl border border-black/5 hover:bg-slate-50 transition-colors group">
            ...
            <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    <div key={i} className="bg-white rounded-xl p-4 border border-outline-variant/10 hover:border-primary/20 transition-all group cursor-pointer tonal-shadow">
                        <div className="flex items-start justify-between mb-4">
                            <div className={`p-2.5 rounded-xl ${statStyles[stat.color as StatColor].iconWrap} group-hover:scale-110 transition-transform`}>
                                <MaterialSymbol icon={stat.icon} filled className="text-xl" />
                            </div>
                            {stat.delta && (
                        ...
                        <div className="flex flex-col">
                            <span className="text-2xl font-headline font-bold text-on-surface leading-tight">
                                {stat.value}
                            </span>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
                            ...
                            {recent_threads.map((thread: ThreadListItem) => {
                                ...
                                return (
                                    <Link key={thread.thread_id} href={`/inbox/${thread.thread_id}`}>
                                        <div className="p-3.5 flex items-center gap-3.5 hover:bg-surface-container-lowest transition-colors cursor-pointer group">
                                            <div className="w-10 h-10 rounded-full bg-surface-container-high flex items-center justify-center text-primary font-bold text-xs shrink-0 border border-white">
                                                {initials}
                                            </div>
                ...
                <div className="xl:col-span-2 flex flex-col gap-5">
                    <div className="flex items-center justify-between px-2">
                        <h2 className="text-lg font-headline font-bold text-on-surface">Intelligence Tasks</h2>
                    ...
                    <div className="bg-surface-container-low rounded-2xl p-5 border border-outline-variant/10 tonal-shadow">
                        <div className="space-y-4">
                            {priority_tasks.length > 0 ? priority_tasks.slice(0, 6).map((task: TaskDTOv1) => (
                                ...
                                <div className="flex items-start gap-3 group cursor-pointer hover:translate-x-1 transition-transform">
                                    <div className={`mt-0.5 w-4 h-4 rounded border-2 border-primary/20 bg-white flex items-center justify-center transition-colors ${task.status === 'COMPLETED' ? 'bg-primary border-primary' : 'group-hover:border-primary/50'}`}>
                                        {task.status === 'COMPLETED' && <MaterialSymbol icon="check" className="text-white text-xs" />}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-[13px] font-bold leading-snug truncate ${task.status === 'COMPLETED' ? 'line-through text-outline' : 'text-on-surface'}`}>
                                            {task.title}
                                        </p>
                        ...
                        <button 
                            ...
                            className="w-full mt-6 py-3 text-[13px] font-bold bg-white text-primary rounded-xl flex items-center justify-center gap-2 border border-outline-variant/10 shadow-sm hover:bg-primary-fixed/20 hover:border-primary/20 transition-all font-headline"
                        >
...
export default function DashboardPage() {
    return (
        <AppShell>
            <Suspense fallback={
                <div className="p-6 space-y-6 animate-pulse w-full max-w-[1280px] mx-auto">
