...
export default function AIUsagePage() {
    ...
    const { data, isLoading, isFetching, isError, refetch } = useQuery<AIUsageResponse>({
        ...
        queryFn: async () => {
            const { data } = await api.get(endpoints.adminMetricsAIUsage, {
                params: {
                    hours,
                    limit: 200,
                },
    ...
    return (
        ...
        function UsageStat({ label, value, sub, icon: Icon, color }: { label: string; value: string; sub: string; icon: any; color: string }) {
            return (
