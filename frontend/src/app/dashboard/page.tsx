...
function DashboardContent() {
        ...
        return (
            <div className="p-4 space-y-4 animate-pulse">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    {[1, 2, 3, 4].map(i => <div key={i} className="h-24 rounded-2xl bg-surface-container-low" />)}
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col p-3 md:p-4 gap-4 max-w-[1180px] mx-auto">
            {/* Header Greeting */}
            <header className="flex flex-col gap-1">
                <h1 className="text-xl md:text-2xl font-headline font-bold text-on-surface tracking-tight">
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {/* AI Briefing Card */}
                <div className="lg:col-span-2 bg-gradient-to-br from-tertiary-fixed/40 to-primary-fixed/20 rounded-2xl p-4 md:p-5 tonal-shadow relative overflow-hidden border border-white/40">
                    <div className="absolute top-0 right-0 p-4 opacity-10 blur-sm">
                        <MaterialSymbol icon="auto_awesome" filled className="text-[64px] text-tertiary" />
                    </div>
                    
                    <div className="relative z-10 flex flex-col gap-3 md:pr-10">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-white/60 flex items-center justify-center text-tertiary shadow-sm border border-white">
                        ...
                        <p className="text-sm md:text-base font-medium text-on-surface leading-snug">
                            {briefing.summary}
                        </p>
                        
                        <div className="flex flex-wrap gap-2 mt-2">
                            {briefing.suggested_actions.map((action: string, i: number) => (
                                <button key={i} className="px-2.5 py-1 bg-white/80 hover:bg-white text-on-surface border border-outline-variant/30 transition-all rounded-full text-[10px] font-bold shadow-sm hover:scale-105 active:scale-95">
                                    {action}
                                </button>
                            ))}
                        </div>
                ...
                <div className="bg-surface-container-low rounded-2xl p-4 border border-outline-variant/10 flex flex-col justify-between tonal-shadow">
                    ...
                    <Link href="/search" className="mt-4 flex items-center justify-between w-full p-2.5 bg-white rounded-xl border border-black/5 hover:bg-slate-50 transition-colors group">
                        <span className="text-xs font-bold text-on-surface">View Insights</span>
                ...
                {[
                    ...
                    <div key={i} className="bg-white rounded-xl p-3 border border-outline-variant/10 hover:border-primary/20 transition-all group cursor-pointer tonal-shadow">
                        ...
                        <div className="flex flex-col">
                            <span className="text-xl font-headline font-bold text-on-surface leading-tight">
                                {stat.value}
                            </span>
                            <span className="text-[10px] font-bold text-outline uppercase tracking-wider mt-1">
                                {stat.label}
                            </span>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
                ...
                <div className="xl:col-span-3 flex flex-col gap-4">
                    <div className="flex items-center justify-between px-2">
                        ...
                        <Link href="/inbox" className="text-xs font-bold text-primary flex items-center gap-1 hover:underline">
                            Open Inbox <MaterialSymbol icon="arrow_outward" className="text-sm" />
                        </Link>
                    </div>

                    <div className="bg-white rounded-2xl border border-outline-variant/10 overflow-hidden tonal-shadow">
                            ...
                            {recent_threads.map((thread: ThreadListItem) => {
                                ...
                                return (
                                    <Link key={thread.thread_id} href={`/inbox/${thread.thread_id}`}>
                                        <div className="p-2.5 flex items-center gap-3 hover:bg-surface-container-lowest transition-colors cursor-pointer group">
                                            <div className="w-8 h-8 rounded-full bg-surface-container-high flex items-center justify-center text-primary font-bold text-[10px] shrink-0 border border-white">
                                                {initials}
                                            </div>
                                            
                                            <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                                                <div className="flex items-center justify-between">
                                                    <h4 className="font-bold text-on-surface text-xs group-hover:text-primary transition-colors truncate">
                                                        {participantName}
                                                    </h4>
                ...
                <div className="xl:col-span-2 flex flex-col gap-4">
                    <div className="flex items-center justify-between px-2">
                        <h2 className="text-base font-headline font-bold text-on-surface">Intelligence Tasks</h2>
                        <span className="text-[10px] font-bold bg-primary-fixed/30 text-primary px-2.5 py-1 rounded-full">
                            {priority_tasks.length} Action Items
                        </span>
                    </div>

                    <div className="bg-surface-container-low rounded-2xl p-4 border border-outline-variant/10 tonal-shadow">
                        <div className="space-y-3">
                            {priority_tasks.length > 0 ? priority_tasks.slice(0, 6).map((task: TaskDTOv1) => (
                                <Link href="/tasks" key={task.task_id} className="block">
                                <div className="flex items-start gap-2.5 group cursor-pointer hover:translate-x-1 transition-transform">
                                    <div className={`mt-0.5 w-3.5 h-3.5 rounded border-2 border-primary/20 bg-white flex items-center justify-center transition-colors ${task.status === 'COMPLETED' ? 'bg-primary border-primary' : 'group-hover:border-primary/50'}`}>
                                        {task.status === 'COMPLETED' && <MaterialSymbol icon="check" className="text-white text-xs" />}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-xs font-bold leading-snug truncate ${task.status === 'COMPLETED' ? 'line-through text-outline' : 'text-on-surface'}`}>
                                            {task.title}
                                        </p>
                                        <div className="flex items-center gap-2 mt-1 text-[9px] font-bold uppercase tracking-widest">
                        ...
                        <button 
                            ...
                            className="w-full mt-4 py-2.5 text-xs font-bold bg-white text-primary rounded-xl flex items-center justify-center gap-2 border border-outline-variant/10 shadow-sm hover:bg-primary-fixed/20 hover:border-primary/20 transition-all font-headline"
                        >
...
export default function DashboardPage() {
    return (
        <AppShell>
            <Suspense fallback={
                <div className="p-4 space-y-4 animate-pulse w-full max-w-[1180px] mx-auto">
                    <div className="h-12 w-64 bg-surface-container rounded-lg" />
                    <div className="h-56 rounded-2xl bg-surface-container w-full" />
                </div>
            }>
                <DashboardContent />
            </Suspense>
